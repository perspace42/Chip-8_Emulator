...
//This action opens a window with a "color picker" that allows the user to choose a color and assigns it to the variable currentColor
void MainWindow::on_actionColor_triggered()
{
    ...
    if(color.isValid()) {
        currentColor = color;//If color is valid, assign it to the currentColor variable
    }
}
...
void MainWindow::on_actionSet_Speed_triggered()
{
    ...
    if(ok){//If ok is selected
        ...
        if(romLoaded && !paused){//If a rom is loaded and the emulator is not paused
